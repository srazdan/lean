...
-- Read rows of comma seperated data either from
...
function nb(file,      t,seen, guess, like, class)
  ...
  function classify(row,         out,like, tmp)
    ...
    for klass,_ in pairs(t.f) do
      ...
      for col,val in pairs(row) do
        if col ~= #row then
          if val ~= "?" then
            tmp = tmp + math.log( like1(klass,col,val) ) end end end
      if tmp > like then
...
-- When we see a new class, add another nested table to `t.f`.
  ...
  function klasses(k)
    if not t.f[k] then
      print("new "..k)
      t.f[k]={}
      for c,_ in pairs(t.header) do
        t.f[k][c] = {} end end 
    return k end

-- When we see new headers, intialize `seen` and `t.attr`.

  function headers(row) do
    for col,_ in pairs(row) do 
      seen[col] = {}
      t.attr[col] = 0 end end
    return row
  end 

  for row, nc, nr in csv(file) do
    t.n = nr
    print("nr "..nr)
    if t.n == 1
      then t.header= headers(row)
      else klass = klasses( row[ nc ] )
           if t.n > 10 then 
             guess,like = classify(row) 
             print(klass, guess,like) 
           end
	         train(row, klass) end end  
end
